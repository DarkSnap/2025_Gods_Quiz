...
class Play:
    def __init__(self, how_many, quiz_type):
        ...
        self.greek_button = Button(self.answer_frame, font=body_font, text="Greek", width=23,
                                   bg="#ff9999", command=partial(self.rounds_results, "Greek", 1))
        self.greek_roman_button_ref.append(self.greek_button)
        self.roman_button = Button(self.answer_frame, font=body_font, text="Roman", width=23,
...
class Stats:
    ...
    def __init__(self, partner, all_stats_info):
        ...
        success_rate = rounds_won / rounds_played * 100 if rounds_played else 0
        ...
        # Create history string for all rounds
        history = ""
        for i in range(rounds_played):
            result = f"Round: {round_number[i]} | Result: {user_scores[i]}"
            history += result + "\n" # Adds existing result to string of results
